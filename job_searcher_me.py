...
def search_me_jobs(keywords: str, location: str, max_jobs: int = 10) -> list:
    """Search Middle East jobs via JSearch API."""
...
def search_all_me(keywords: str, max_per_location: int = 10) -> list:
    """Search across all Middle East locations."""
    all_jobs = []

    for location in ME_LOCATIONS:
        jobs = search_me_jobs(keywords, location, max_per_location)
        all_jobs += jobs

    print(f"\n📦 Total Middle East jobs collected: {len(all_jobs)}")
    return all_jobs
