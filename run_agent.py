...
def main():
    print("=" * 60)
    print(f"JOB AGENT — Remote Phase | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 60)

    # Step 1: Scrape
    all_jobs = search_all(SEARCH_KEYWORDS, MAX_PER_SOURCE)

    # Step 2: Filter already seen
    new_jobs = filter_new_jobs(all_jobs)

    if not new_jobs:
        print("\n✅ No new jobs today. Try again tomorrow.")
        return
    ...
    # Step 4: Mark all new jobs as seen (even unmatched ones)
    ...
    print("\n" + "=" * 60)
    print(f"TOP MATCHES ({len(matched_jobs)} jobs)")
    print("=" * 60)
